...
int DELAY_LOOP_COUNT=1000000000;	// Valor global tope del bucle for()
...
static void vASimpleTask( void * pvParameters ){
	...
	for(;;)
	{
		...
		for(delay=0;delay < DELAY_LOOP_COUNT; delay++ ){}	// Funciona como un delay, recorre el bucle for() 'DELAY_LOOP_COUNT' veces
	}
}
